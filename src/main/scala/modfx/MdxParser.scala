...
object MdxParser {
  ...
  object Ast {
    ...
    trait Nesting extends Node {
      def nested: Seq[Node]
      def nesting: Seq[Nesting] = nested.flatMap{
    ...
    case class TopNode(name: String, descriptor: Option[String], nested: Seq[Node]) extends Nesting
    case class NestedNode(name: String, descriptor: Option[String], nested: Seq[Node]) extends Nesting
  ...
  object BaseParsers {
    ...
    val path = P( name ~ ("." ~ name).rep(0) )
    val fieldTypes = List("string", "date", "byte", "short", "datetime", "money", "int", "boolean")
  ...
  def parseToAst(input: String): Either[String, Seq[Ast.TopNode]] = {
    ...
    val objDescriptor = P( CharIn("~=:") | ("(" ~ (StringIn(objTypes:_*) | ("base " ~ name)) ~ ")") )
    ...
    val nestedNode: Parser[Node] = P( name.! ~ (" ".rep(1) ~ "(" ~ nestedDescriptor.! ~ ")").? )
      .map{ case (nme, dsc) => NestedNode(nme, dsc, Seq.empty) }
    def nestedBranch(level: Int): Parser[Node] = P( nestedNode ~ nestedList(level + 1) )
      .map{ case (nn, subNested) => NestedNode(nn.name, nn.descriptor, subNested) }
    ...
    val obj = P( objHeader ~ nestedList(1) ).map{ case (nme, tpe, nst) => TopNode(nme, tpe, nst) }
